...
object PatternMatcher {
  ...
  val TypeSwitchKey: Key[TypeSwitchInfo] = new StickyKey[TypeSwitchInfo]

  case class TypeSwitchInfo(
    isEnumBootstrap: Boolean,
    selectorTpe: Type,
    labels: List[TypeSwitchLabel],
    hasNullCase: Boolean
  )

  sealed abstract class TypeSwitchLabel

  case class ClassLabel(tpe: Type) extends TypeSwitchLabel

  case class StringLabel(value: String) extends TypeSwitchLabel

  case class IntegerLabel(value: Int) extends TypeSwitchLabel
  ...
  class Translator(resultType: Type, thisPhase: MiniPhase)(using Context) {
    ...
    private def targetVersion(using Context): Int =
      val s = ctx.settings
      val release =
        Option(s.javaOutputVersion.value).filter(_.nonEmpty)
      val target =
        Option(s.XuncheckedJavaOutputVersion.value).filter(_.nonEmpty)
      release.orElse(target)
        .flatMap(_.toIntOption)
        .getOrElse(17)

    private def typeSwitchIndyEnabled(using Context): Boolean =
      ctx.settings.YemitTypeSwitchIndy.value && targetVersion >= 21

    private def isTypeSwitchSelectorType(tpe: Type): Boolean =
      tpe.widen <:< defn.ObjectType

    private def isNullPat(pat: Tree): Boolean = pat match {
      case Literal(Constant(null)) => true
      case _ => false
    }

    private def isEnumValuePat(pat: Tree): Boolean =
      val sym = pat.symbol
      sym.isAllOf(Flags.EnumValue)
      || (sym.is(Flags.Enum) && sym.is(Flags.JavaStatic))

    private def typeSwitchConstLabel(pat: Tree): Option[TypeSwitchLabel] = pat match {
      case Literal(Constant(s: String)) => Some(StringLabel(s))
      case Literal(Constant(i: Int))    => Some(IntegerLabel(i))
      case sel if isEnumValuePat(sel)   => Some(StringLabel(sel.symbol.name.toString))
      case _                            => None
    }

    private def isTypeSwitchConstPat(pat: Tree): Boolean =
      isNullPat(pat) || typeSwitchConstLabel(pat).isDefined


    private def collectTypeSwitchCases(scrutinee: Tree, plan: SeqPlan): List[(Tree, Plan)] =
      val selectorIsJavaEnum =
        scrutinee.tpe.widen.classSymbol.derivesFrom(defn.JavaEnumClass)
      def usableConst(tree: Tree): Boolean =
        if isEnumValuePat(tree) then selectorIsJavaEnum
        else isTypeSwitchConstPat(tree)

      def recur(plan: Plan): List[(Tree, Plan)] = plan match
        case SeqPlan(TestPlan(TypeTest(tpt, _), scrut, _, ons), tail)
        if scrut === scrutinee && !canFallThrough(ons) =>
          (tpt, ons) :: recur(tail)
        case SeqPlan(TestPlan(EqualTest(tree), scrut, _, ons), tail)
        if scrut === scrutinee && !canFallThrough(ons) && usableConst(tree) =>
          (tree, ons) :: recur(tail)
        case _ =>
          (EmptyTree, plan) :: Nil

      if isTypeSwitchSelectorType(scrutinee.tpe) then recur(plan) else Nil

    private def planHasGuardTest(plan: Plan): Boolean = plan match
      case TestPlan(GuardTest, _, _, _) => true
      case LetPlan(_, body)             => planHasGuardTest(body)
      case SeqPlan(head, _)             => planHasGuardTest(head)
      case _                            => false

    private def isTriggerCase(c: (Tree, Plan)): Boolean =
      val (pat, ons) = c
      if pat.isEmpty then false
      else if isNullPat(pat) then true
      else if isEnumValuePat(pat) then true
      else if typeSwitchConstLabel(pat).isDefined then planHasGuardTest(ons)
      else true

    private def hasEnoughTypeSwitchCases(cases: List[(Tree, Plan)]): Boolean =
      cases.count { case (pat, _) => !pat.isEmpty } >= MinSwitchCases - 1

    private def hasSupportedNullOrder(cases: List[(Tree, Plan)]): Boolean =
      val realCases = cases.filter { case (pat, _) => !pat.isEmpty }
      realCases.indexWhere { case (pat, _) => isNullPat(pat) } match
        case -1 | 0 => true
        case _ => false

    private def typeSwitchLabelKey(label: TypeSwitchLabel): Option[(String, Any)] = label match
      case ClassLabel(tpe) =>
        val cls = tpe.widen.classSymbol
        if cls.exists then Some(("class", cls)) else None
      case StringLabel(value) =>
        Some(("string", value))
      case IntegerLabel(value) =>
        Some(("int", value))

    private def hasSupportedUniqueLabels(cases: List[(Tree, Plan)]): Boolean =
      val keys = cases.flatMap { case (pat, _) => patToLabel(pat) }.map(typeSwitchLabelKey)
      keys.forall(_.isDefined) && keys.flatten.distinct.size == keys.size

    private def isTypeSwitchEligible(scrutinee: Tree, plan: SeqPlan): (Boolean, List[(Tree, Plan)]) = {
      if !typeSwitchIndyEnabled then return (false, Nil)

      val tpe = scrutinee.tpe.widen
      if tpe.isPrimitiveValueType && tpe =:= defn.NullType then return (false, Nil)

      val cases = collectTypeSwitchCases(scrutinee, plan)
      if cases.isEmpty then return (false, Nil)

      val widenedSel = scrutinee.tpe.widen
      if widenedSel.classSymbol.is(Flags.Enum) && !widenedSel.classSymbol.derivesFrom(defn.JavaEnumClass) then
        return (false, Nil)

      if !hasSupportedNullOrder(cases) then return (false, Nil)

      if !hasSupportedUniqueLabels(cases) then return (false, Nil)

      val triggers = cases.filter(isTriggerCase)
      if triggers.isEmpty then return (false, Nil)

      if !hasEnoughTypeSwitchCases(cases) then return (false, Nil)

      (true, cases)
    }

    private def patToLabel(pat: Tree): Option[TypeSwitchLabel] = pat match
      case EmptyTree => None
      case Literal(Constant(null)) => None
      case _ => typeSwitchConstLabel(pat).orElse(Some(ClassLabel(pat.tpe)))

    private def computeTypeSwitchInfo(selectorTpe: Type, cases: List[(Tree, Plan)]): TypeSwitchInfo =
      val widenedSel = selectorTpe.widen
      val isEnum = widenedSel.classSymbol.derivesFrom(defn.JavaEnumClass)
      val hasNullCase = cases.exists { case (pat, _) => isNullPat(pat) }
      val labels: List[TypeSwitchLabel] = cases.flatMap { case (pat, _) => patToLabel(pat) }
      TypeSwitchInfo(isEnum, widenedSel, labels, hasNullCase)

    private def caseDefOf(pat: Tree, ons: Plan, scrutineeTpe: Type): CaseDef = pat match {
      case EmptyTree =>
        CaseDef(Underscore(scrutineeTpe.widen), EmptyTree, emit(ons))
      case Literal(Constant(null)) =>
        CaseDef(pat, EmptyTree, emit(ons))
      case _ if typeSwitchConstLabel(pat).isDefined =>
        CaseDef(pat, EmptyTree, emit(ons))
      case tpt =>
        CaseDef(Typed(Underscore(tpt.tpe), tpt), EmptyTree, emit(ons))
    }

    private def emitTypeSwitchMatch(scrutinee: Tree, cases: List[(Tree, Plan)]): Match = {
      val info     = computeTypeSwitchInfo(scrutinee.tpe, cases)
      val nullCase     = cases.find   { case (pat, _) => isNullPat(pat) }
      val defaultCase  = cases.find   { case (pat, _) => pat.isEmpty }
      val contentCases = cases.filter { case (pat, _) => !isNullPat(pat) && !pat.isEmpty }
      val ordered      = nullCase.toList ::: contentCases ::: defaultCase.toList
      val caseDefs = ordered.map { case (pat, ons) => caseDefOf(pat, ons, scrutinee.tpe) }
      val matchTree = Match(scrutinee, caseDefs)
      matchTree.pushAttachment(TypeSwitchKey, info)
      matchTree
    }
    ...
    private def emit(plan: Plan): Tree = {
      ...
      plan match {
        ...
        case plan: SeqPlan =>
          ...
          def maybeEmitTypeSwitch(scrutinee: Tree): Tree = {
            val (isEligible, typeSwitchCases) =
              isTypeSwitchEligible(scrutinee, plan)
            if isEligible then
              emitTypeSwitchMatch(scrutinee, typeSwitchCases)
            else
              maybeEmitSwitch(scrutinee)
          }
          plan.head match {
            case testPlan: TestPlan =>
              maybeEmitTypeSwitch(testPlan.scrutinee)
